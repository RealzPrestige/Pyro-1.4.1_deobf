...
public class HauseHop extends SpeedMode
{
    ...
    public void onMotionUpdates(final EventPlayerMotionUpdate event) {
        ...
        final double xDist = HauseHop.mc.player.posX - HauseHop.mc.player.prevPosX;
        final double zDist = HauseHop.mc.player.posZ - HauseHop.mc.player.prevPosZ;
        this.lastDist = Math.sqrt(xDist * xDist + zDist * zDist);
        if (!MovementUtils.isMoving()) {
            this.safeJump = true;
        }
        else if (HauseHop.mc.player.onGround) {
            this.safeJump = false;
        }
    }
    ...
    public void onPlayerMove(final EventPlayerMove event) {
        ...
        if (this.timerDelay != 0) {
            ...
            final EntityPlayerSP player = HauseHop.mc.player;
            player.motionX *= 1.0199999809265137;
            final EntityPlayerSP player2 = HauseHop.mc.player;
            player2.motionZ *= 1.0199999809265137;
        }
        if (HauseHop.mc.player.onGround && MovementUtils.hasMotion()) {
            this.level = 2;
        }
        if (this.round(HauseHop.mc.player.posY - (int)HauseHop.mc.player.posY) == this.round(0.138)) {
            final EntityPlayerSP player3 = HauseHop.mc.player;
            player3.motionY -= 0.08;
            event.setY(event.getY() - 0.09316090325960147);
            final EntityPlayerSP player4 = HauseHop.mc.player;
            player4.posY -= 0.09316090325960147;
        }
        if (this.level == 1 && (HauseHop.mc.player.moveForward != 0.0f || HauseHop.mc.player.moveStrafing != 0.0f)) {
            this.level = 2;
            this.moveSpeed = 1.38 * getDefaultSpeed() - 0.01;
        }
        else if (this.level == 2) {
            this.level = 3;
            event.setY(HauseHop.mc.player.motionY = 0.399399995803833);
            ...
            if (HauseHop.mc.world.getCollisionBoxes((Entity)HauseHop.mc.player, HauseHop.mc.player.getEntityBoundingBox().offset(0.0, HauseHop.mc.player.motionY, 0.0)).size() > 0 || HauseHop.mc.player.collidedVertically) {
                this.level = 1;
            }
            this.moveSpeed = this.lastDist - this.lastDist / 159.0;
        }
        this.moveSpeed = Math.max(this.moveSpeed, SpeedMode.getDefaultSpeed());
        this.moveSpeed = Math.min(this.moveSpeed, (this.ticks > 25) ? 0.449 : 0.433);
        float forward = HauseHop.mc.player.movementInput.moveForward;
        float strafe = HauseHop.mc.player.movementInput.moveStrafe;
        float yaw = HauseHop.mc.player.rotationYaw;
        ++this.ticks;
        if (this.ticks > 50) {
            this.ticks = 0;
        }
        ...
        HauseHop.mc.player.stepHeight = 0.6f;
        if (forward == 0.0f && strafe == 0.0f) {
            event.setX(0.0);
            event.setZ(0.0);
        }
    ...
    private Block getBlock(final AxisAlignedBB axisAlignedBB) {
        for (int x = MathHelper.floor(axisAlignedBB.minX); x < MathHelper.floor(axisAlignedBB.maxX) + 1; ++x) {
            for (int z = MathHelper.floor(axisAlignedBB.minZ); z < MathHelper.floor(axisAlignedBB.maxZ) + 1; ++z) {
                final Block block = HauseHop.mc.world.getBlockState(new BlockPos(x, (int)axisAlignedBB.minY, z)).getBlock();
                if (block != null) {
                    return block;
                }
            }
    ...
    private Block getBlock(final double offset) {
        return this.getBlock(HauseHop.mc.player.getEntityBoundingBox().offset(0.0, offset, 0.0));
    }
